...
@section('content')
@php
    // Helper function terpusat untuk memproses path / URL / Base64 gambar paket
    $getSrcImage = function($img) {
        if (!$img) return null;
        if (is_resource($img)) $img = stream_get_contents($img);
        if (str_starts_with($img, 'http') || str_starts_with($img, 'data:')) return $img;
        if (str_starts_with($img, 'storage/') || str_starts_with($img, 'uploads/')) return asset($img);
        return 'data:image/jpeg;base64,' . base64_encode($img);
    };

    $bannerImage = $getSrcImage($package->image);
@endphp

    {{-- HERO BANNER UTAMA DINAMIS --}}
    <section class="relative h-[50vh] md:h-[60vh] bg-slate-950 text-white overflow-hidden">
        @if($bannerImage)
            <img src="{{ $bannerImage }}" 
                 alt="{{ $package->title }}" 
        ...
        <div class="absolute bottom-0 inset-x-0 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            <div class="flex flex-wrap gap-6 items-center text-slate-200 text-xs md:text-sm mt-4 font-medium">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-12 items-start relative z-10">
            ...
            <div class="lg:col-span-2 space-y-8">
                ...
                <section class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/60 shadow-md shadow-slate-100">
                    <div class="flex items-center gap-3 mb-6">
                        <div class="w-10 h-10 rounded-xl bg-linear-to-tr from-sky-600 to-sky-400 text-white flex items-center justify-center shadow-md shadow-sky-100">
                            <i class="fas fa-file-alt text-sm"></i>
                        </div>
                        <h2 class="text-xl font-black text-slate-900 tracking-tight">Deskripsi Paket</h2>
                    </div>
                ...
                <section class="bg-white p-6 md:p-8 rounded-3xl border border-slate-200/60 shadow-md shadow-slate-100 relative overflow-hidden">
                    ...
                    <div class="flex items-center gap-3 mb-8 relative z-10">
                        ...
                        <h2 class="text-xl font-black text-slate-900 tracking-tight">Rencana Perjalanan</h2>
                    </div>
                            ...
                            @foreach($rawDays as $dayText)
                                ...
                                @if(!empty($dayText))
                                    ...
                                    <div class="relative group">
                                        <div class="absolute -left-10.5 top-0 bg-linear-to-br from-sky-400 to-sky-600 text-white w-9 h-9 rounded-xl flex items-center justify-center text-xs font-black shadow-lg shadow-sky-200 group-hover:scale-110 transition-transform duration-300">
                                            {{ $dayCounter }}
                                        </div>
                    ...
                    @php
                        $parseList = function($data) {
                            ...
                            if (is_string($data)) {
                                ...
                                return array_filter(array_map('trim', $split));
                            }
                            return [];
                        };

                        $includesList = $parseList($package->includes);
                        $excludesList = $parseList($package->excludes);
                    @endphp
                        ...
                        <ul class="space-y-3 text-slate-600 text-xs md:text-sm">
                            @forelse($includesList as $include)
                                <li class="flex items-start gap-3 bg-emerald-50/40 p-3 rounded-xl border border-emerald-100/50">
                                    ...
                                    <span class="font-bold text-slate-700">{{ rtrim($include, '.') }}.</span>
                                </li>
                            @empty
                        ...
                        <ul class="space-y-3 text-slate-600 text-xs md:text-sm">
                            @forelse($excludesList as $exclude)
                                <li class="flex items-start gap-3 bg-rose-50/40 p-3 rounded-xl border border-rose-100/50">
                                    ...
                                    <span class="font-bold text-slate-700">{{ rtrim($exclude, '.') }}.</span>
                                </li>
                            @empty
                ...
                <section class="space-y-4">
                    <div class="flex items-center justify-between bg-slate-900 p-4 md:p-5 rounded-2xl text-white shadow-md">
                        <div class="flex items-center gap-3">
                            <i class="fas fa-comments text-sky-400 text-lg"></i>
                            <h2 class="text-base md:text-lg font-black tracking-tight">
                                Ulasan Wisatawan ({{ $package->reviews ? $package->reviews->count() : 0 }})
                            </h2>
                        </div>
                            ...
                            <div class="p-5 rounded-2xl border border-slate-200/60 bg-white shadow-xs space-y-4 transition-all hover:-translate-y-1 hover:border-sky-300 hover:shadow-md flex flex-col justify-between">
                                <div class="space-y-3">
                                    <div class="flex items-center justify-between gap-4">
                                        <div class="flex items-center gap-3">
                                            <div class="w-10 h-10 rounded-xl bg-linear-to-tr from-sky-400 to-sky-600 text-white font-black text-xs flex items-center justify-center uppercase tracking-wider shadow-sm">
                                            ...
                                            <div>
                                                ...
                                                <p class="text-[10px] text-slate-400 mt-0.5 font-medium">{{ $review->created_at ? $review->created_at->diffForHumans() : 'Baru saja' }}</p>
                                            </div>
                                        </div>
            ...
            <div class="lg:sticky lg:top-24 space-y-6 relative z-10">
                <div class="bg-white rounded-3xl border border-slate-200/60 shadow-xl shadow-slate-200/50 overflow-hidden">
                    ...
                    <div class="divide-y divide-slate-100 text-sm">
                        <div class="p-4 flex justify-between items-center bg-slate-50">
                            <span class="font-bold text-slate-900">Peserta</span>
                            <span class="font-bold text-slate-900">Tarif / Pax</span>
                        </div>
                        ...
                        <div class="p-4 flex justify-between items-center bg-sky-50/50">
                            <span class="font-bold text-slate-900 flex items-center gap-1.5">
                                4 - 6 Orang <span class="bg-linear-to-r from-sky-400 to-sky-600 text-white text-[9px] font-black px-1.5 py-0.5 rounded-sm uppercase tracking-wider">Best Value</span>
                            </span>
                    ...
                    <div class="p-6 bg-slate-50 border-t border-slate-100 space-y-4">
                        <form action="{{ route('booking.form', ['id' => $package->id]) }}" method="GET">
                            <button type="submit" class="w-full bg-linear-to-r from-sky-500 to-sky-600 hover:from-sky-600 hover:to-sky-700 text-white font-black text-sm text-center py-4 px-4 rounded-xl shadow-lg shadow-sky-200 transition-all flex items-center justify-center gap-3 cursor-pointer tracking-wider">
                                PESAN SEKARANG
                            </button>
                        </form>
                ...
                <div class="p-5 rounded-2xl border border-slate-200/60 bg-white flex items-center gap-4 shadow-xs">
                    ...
                    <div>
                        <h4 class="text-xs font-black text-slate-900">Paket Kustom Tersedia</h4>
