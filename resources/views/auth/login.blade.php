        ...
        <div class="w-full max-w-5xl bg-white md:rounded-3xl shadow-2xl shadow-sky-950/20 flex overflow-hidden border border-sky-100">
            ...
            <div class="w-full md:w-1/2 p-8 sm:p-12 md:p-10 lg:p-12 flex flex-col justify-center relative bg-white">
                ...
                <form action="{{ route('login') }}" method="POST" class="space-y-5">
                    ...
                    <div>
                        <label class="block text-[10px] font-black uppercase tracking-wider text-slate-600 mb-1.5">Alamat Email</label>
                        <div class="relative">
                            ...
                            <input type="email" name="email" value="{{ old('email') }}" required placeholder="contoh@email.com"
                                   class="w-full text-xs pl-11 pr-4 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/5 transition-all @error('email') @enderror">
                        </div>
                        @error('email') <span class="text-[10px] text-red-500 font-medium mt-1.5 block">{{ $message }}</span> @enderror
                    </div>

                    <div x-data="{ showPassword: false }">
                        <label class="block text-[10px] font-black uppercase tracking-wider text-slate-600 mb-1.5">Password</label>
                        <div class="relative">
                            ...
                            <input :type="showPassword ? 'text' : 'password'" 
                                ...
                                required 
                                placeholder="••••••••"
                                class="w-full text-xs pl-11 pr-11 py-3.5 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:border-sky-500 focus:bg-white focus:ring-4 focus:ring-sky-500/5 transition-all">
