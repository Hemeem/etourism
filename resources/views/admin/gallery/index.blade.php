...
<div x-data="{ openAddModal: false, openEditModal: false, openDeleteModal: false, editId: '', editTitle: '', editCategory: '', editSource: '', editAction: '', deleteAction: '' }">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
        @forelse($galleries as $gallery)
            @php
                $imagesArray = json_decode($gallery->image, true) ?? [];
                $coverImage = !empty($imagesArray) ? $imagesArray[0] : '';
            @endphp
            ...
            <div class="bg-white rounded-2xl border border-slate-100 shadow-sm overflow-hidden group hover:shadow-xl transition-all duration-300">
                <div class="relative aspect-video bg-slate-100 overflow-hidden">
                    @if($coverImage)
                        <img src="data:image/jpeg;base64,{{ $coverImage }}" alt="{{ $gallery->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500">
                    @else
                    ...
                    @if(count($imagesArray) > 1)
                        <span class="absolute top-3 left-3 bg-slate-900/70 backdrop-blur-xs text-white text-[10px] font-bold px-2 py-1 rounded-lg">
                            <i class="fas fa-images mr-1"></i> {{ count($imagesArray) }} Foto
                        </span>
                    @endif
                    ...
                    <div class="absolute inset-0 bg-linear-to-t from-slate-950/70 via-transparent to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4 gap-2 justify-end">
                        <button @click="openEditModal = true; editId = '{{ $gallery->id }}'; editTitle = @js($gallery->title); editCategory = '{{ $gallery->filter_category }}'; editSource = @js($gallery->source); editAction = '{{ route('admin.galleries.update', $gallery->id) }}'" 
    ...
    <div x-show="openAddModal" class="fixed inset-0 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" x-transition x-cloak>
        <div @click.away="openAddModal = false" class="bg-white rounded-2xl shadow-2xl max-w-md w-full border border-slate-100 overflow-hidden">
            ...
            <form action="{{ route('admin.galleries.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">Pilih File Gambar (Bisa pilih lebih dari 1)</label>
                    <input type="file" name="images[]" multiple required class="w-full text-xs text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-black file:uppercase file:tracking-wider file:bg-sky-50 file:text-sky-700 hover:file:bg-sky-100 cursor-pointer">
                </div>
    ...
    <div x-show="openEditModal" class="fixed inset-0 bg-slate-950/40 backdrop-blur-sm flex items-center justify-center z-50 p-4" x-transition x-cloak>
        <div @click.away="openEditModal = false" class="bg-white rounded-2xl shadow-2xl max-w-md w-full border border-slate-100 overflow-hidden">
            ...
            <form :action="editAction" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div>
                    <label class="block text-[10px] font-black text-slate-400 uppercase tracking-wider mb-1.5">Ganti/Tambah Gambar (Opsional)</label>
                    <input type="file" name="images[]" multiple class="w-full text-xs text-slate-500 file:mr-4 file:py-2.5 file:px-4 file:rounded-xl file:border-0 file:text-xs file:font-black file:uppercase file:tracking-wider file:bg-sky-50 file:text-sky-700 hover:file:bg-sky-100 cursor-pointer">
                    <span class="text-[10px] text-slate-400 mt-1 block">*Kosongkan jika tidak ingin mengganti/memperbarui file gambar</span>
                </div>
                <div class="pt-2 flex justify-end gap-2">
