...
<head>
    ...
    <style>
        /* Sumbu Utama Tata Letak Kertas A4 */
        @page {
            size: A4 portrait;
            margin: 20mm 20mm 20mm 20mm; /* Batas margin kertas keliling 2cm */
        }
        ...
        /* Pembungkus Dokumen agar Tidak Meluber Mepet ke Samping */
        .print-container {
            width: 100%;
            max-width: 170mm; /* Kertas A4 (210mm) - Margin Kanan Kiri (40mm) = Lebar Konten Maksimal 170mm */
            margin: 0 auto;
            padding: 0;
...
<body>
    ...
    <div class="print-container">
        ...
        <div class="cop-surat">
            ...
            <p class="periode">Periode Data: {{ \Carbon\Carbon::parse($startDate)->format('d F Y') }} s/d {{ \Carbon\Carbon::parse($endDate)->format('d F Y') }}</p>
        ...
        <table>
            ...
            <tbody>
                <tr>
                    ...
                    <td class="text-emerald">Rp {{ number_format($totalRevenue, 0, ',', '.') }}</td>
                </tr>
                <tr>
                    <td>Transaksi Menunggu Pembayaran (Unpaid)</td>
                    <td>{{ $reservations->where('status', 'unpaid')->count() }} Invoice</td>
                    <td>Rp {{ number_format($reservations->where('status', 'unpaid')->sum('total_price'), 0, ',', '.') }}</td>
                </tr>
                <tr>
        ...
        <table>
            <thead>
                <tr>
                    ...
                    <th style="width: 20%">Nama Wisatawan</th>
                    <th style="width: 24%">Paket Destinasi Wisata</th>
            ...
            <tbody>
                @foreach($reservations as $res)
                    <tr>
                        ...
                        <td>{{ $res->created_at->format('d/m/Y H:i') }} WIB</td>
        ...
        <div class="kotak-omset">
            ...
            <div class="nilai">Rp {{ number_format($totalRevenue, 0, ',', '.') }}</div>
        </div>

        <div class="lembar-pengesahan">
            <p>Batam, {{ now()->format('d F Y') }}</p>
