...
<div x-data="{ 
                        ...
                        <td class="py-4 px-6">
                            <div class="flex items-center gap-4">
                                <div class="w-14 h-10 rounded-lg bg-slate-100 border border-slate-200 flex items-center justify-center overflow-hidden shrink-0">
                                    @if($item->image)
                                        <img src="data:image/jpeg;base64,{{ $item->image }}" class="w-full h-full object-cover" alt="{{ $item->title }}">
                                    @else
    ...
    {{-- TAMBAH DATA (CREATE) --}}
    <div x-show="openCreateModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-xs shadow-2xl overflow-y-auto" x-transition x-cloak>
            ...
            <form action="{{ route('admin.news.store') }}" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        ...
                        <input type="text" name="title" required placeholder="Contoh: 5 Kuliner Wajib Belitung" class="w-full text-xs px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-sky-500 shadow-inner">
                    </div>
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-slate-700 uppercase tracking-widest block">Kategori Artikel/Berita</label>
                        <input type="text" name="category" placeholder="Contoh: Destinasi, Kuliner, Tips, Budaya" class="w-full text-xs px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none focus:border-sky-500 shadow-inner">
                    </div>
                </div>
                ...
                <div class="space-y-1.5">
                    ...
                    <textarea name="content" rows="10" required placeholder="Tulis isi berita atau edukasi wisata di sini secara lengkap..." class="w-full text-xs px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none resize-none shadow-inner"></textarea>
                </div>
    ...
    <div x-show="openEditModal" class="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-950/40 backdrop-blur-xs overflow-y-auto" x-transition x-cloak>
        <div @click.away="openEditModal = false" class="bg-white rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto border border-slate-100 shadow-2xl">
            ...
            <form :action="'/admin/news/' + editData.id" method="POST" enctype="multipart/form-data" class="p-6 space-y-4">
                ...
                <div class="grid grid-cols-2 gap-4">
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-slate-700 uppercase tracking-widest block">Judul Artikel</label>
                    ...
                    <div class="space-y-1.5">
                        <label class="text-[10px] font-black text-slate-700 uppercase tracking-widest block">Kategori Wisata</label>
                        <input type="text" name="category" x-model="editData.category" class="w-full text-xs px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none">
                    </div>
                </div>
                ...
                <div class="space-y-1.5">
                    <label class="text-[10px] font-black text-slate-700 uppercase tracking-widest block">Isi Konten Artikel</label>
                    <textarea name="content" rows="10" x-model="editData.content" required class="w-full text-xs px-4 py-3 bg-white border border-slate-200 rounded-xl focus:outline-none resize-none"></textarea>
                </div>
